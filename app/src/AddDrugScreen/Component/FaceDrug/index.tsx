...
import selectionSlice from "../../../../modal/Selection/selectionSlice";
import faceDrugSlice, { getFaceImg } from "./faceDrugSlice";
...
function FaceDrug () {
    ...
    const setUri = faceDrugSlice.actions.setUri;
    const setLoading = loadingSlice.actions.setShow;
   
    const handleShowModal = useCallback(() => {
        dispatch(setShowChooseModal({show:true, controller:'add'}));
    },[]);
    ...
    const removeImage = useCallback(() => {
        dispatch(setChoose(null));
        dispatch(setUri({uri:null}));
    },[]);

    useEffect(() => {
        if(selection.choose) {
            switch (selection.choose) {
                case 'pickImage': 
            ...
            dispatch(setChoose(null))
            dispatch(setShowChooseModal({show:false, controller:null}));
        }  
    },[selection.choose])
