...
import { RootStackParamList, addScreenParam, addScreenParamProp } from '../../../../../navigation/stack/rootStackParamlist';
...
const FormAdd: React.FC<FormAddProps>  = ({values, errors, setValues, handleBlur, handleChange}) : React.JSX.Element  => {
    ...
    const route = useRoute<RouteProp<RootStackParamList,'addScreen'>>();

    const reset = formSlice.actions.reset;

    useEffect(() => {
      if(route.params) {
        setValues({...values,'MSSP':route.params.code})
      }
    },[route.params]);
